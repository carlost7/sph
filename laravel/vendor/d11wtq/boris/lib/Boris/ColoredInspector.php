...
class ColoredInspector implements Inspector
{
      ...
      {
            return preg_replace(
                    '/^/m', $this->_colorize('comment', '// '), $this->_dump($variable)
            );
      }
      ...
      private function _dumpObject($value)
      {
            return $this->_dumpStructure(
                            sprintf('object(%s)', get_class($value)), $this->objectVars($value)
            );
      }
      ...
      public function _buildAst($type, $value, $seen = array())
      {
            ...
            return array(
                ...
                'children' => empty($vars) ? array() : array_combine(
                                array_map(array($this, '_dump'), array_keys($vars)), array_map(
                                        function($v) use($self, $nextSeen)
                                {
                                      if (is_object($v))
                                      {
                                            return $self->_buildAst(
                                                            sprintf('object(%s)', get_class($v)), $v, $nextSeen
                                            );
                                      }
                                      elseif (is_array($v))
                                      {
            ...
            return implode(
                ...
                implode(
                                ...
                                function($k) use($self, $children, $indent)
                        {
                              ...
                              {
                                    return sprintf(
                                            '%s%s => %s', str_repeat(' ', ($indent + 1) * 2), $k, $self->_astToString($children[$k], $indent + 1)
                                    );
                              }
                              else
                              {
                                    return sprintf(
                                            '%s%s => %s', str_repeat(' ', ($indent + 1) * 2), $k, $children[$k]
                                    );
                              }
                        }, array_keys($children)
                        )
      ...
      private function _colorize($type, $value)
      {
            ...
            return sprintf(
                    "%s%s\033[0m", static::$TERM_COLORS[$colorName], $value
            );
      }
